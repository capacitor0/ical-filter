  ...
  new Response(
    ...
    <html>
      <head>
        ...
        <title>Basic iCalendar filter tool</title>
        <link rel="stylesheet" href="https://unpkg.com/mvp.css" />
      </head>
      <body>
        <header>
          <h1>iCalendar event filtering</h1>
          <p>Filter a iCalendar event based on it's content.</p>
          <p>
            <i
              >Lost? Checkout
              <a href="https://github.com/capacitor0/ical-filter"
                >README.md</a
              >
        ...
        <main style="padding-top: 0px">
          <section>
            <form>
              <label for="link">Link to calendar</label>
              <input
                type="text"
                name="link"
                id="link"
                placeholder="webcal://foobar.bar/calendar.ics"
                autocomplete="off"
                required
              />
              <label for="user">Event must contain (string)</label>
              <input
                type="text"
                name="user"
                id="user"
                ...
                required
              />
              <button>Generate link to filtered calendar</button>
            </form>
          </section>
      ...
      <script>
        ...
        const handleSubmit = async (evt) => {
          ...
          const url = \`\${window.location.href}ics?calendarURL=\${encodeURIComponent(linkInput.value)}&summaryFilter=\${encodeURIComponent(userInput.value)}\`;
